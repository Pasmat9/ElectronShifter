...
function definujPocetHodinKOdpracovani (){
  ...
  const hodinKOdpracovani = polePouzeVsednichDnu.length * 7.5;
  ...
  let dataKulozeni = { 
    ...
    zamestnanci: Array.from(datoveRadky).map(row => {
      return {
        ...
        hodinyPrevod: row.querySelector('.prevodHodinDal').textContent};
    })}
...
function renderKalendar(rok, mesic) {
  ...
  for (let prazdneBunkyZaKalendarem = 0; prazdneBunkyZaKalendarem<pocetSloupcuKonceKalendare; prazdneBunkyZaKalendarem++){
    denVTabulce.appendChild(pridejBunku());
    if (prazdneBunkyZaKalendarem===0){
      const bunka = pridejBunku()
      bunka.classList.add('bunkaProHodinyKOdpracovani');
...
function spoctiZbyleHodiny(radek){
  ...
  const zbyleHodiny = hodinCelkem-nutnoOdpracovat;
  return radek.children[pocetDniVMesici+indexSloupceZacatkuKalendare+1].textContent = zbyleHodiny
}
...
function zamichejDostupneLidi (dostupniLide) {
  const hodinKOdpracovani = definujPocetHodinKOdpracovani()

  dostupniLide.sort((a, b) => {
    const uvazekA = parseFloat(a.children[indexUvazek].textContent)*hodinKOdpracovani || hodinKOdpracovani;
    const uvazekB = parseFloat(b.children[indexUvazek].textContent)*hodinKOdpracovani || hodinKOdpracovani;
...
function generujSmeny() {
  ...
  radky.forEach(row=>{
    spoctiHodiny(row);
    spoctiZbyleHodiny(row);
  })
  const bunkaProHodinyKOdpracovani = document.querySelector('.bunkaProHodinyKOdpracovani');
  bunkaProHodinyKOdpracovani.textContent = -definujPocetHodinKOdpracovani();
};
